...
function loadModule(force = false): SqlJsModule {
  if (_module && !force) return _module;
  const SQL = _module?.SQL ?? null;
  // top-level await in module-init is the original approach; preserve it.
  return _module as unknown as SqlJsModule;
}

/** Lazy initialization used at boot. */
export async function initRepository(configOverride: Partial<AppConfig> = {}): Promise<typeof repository> {
  _config = { ...getConfig(), ...configOverride };
  await ensureLoaded();
  return repository;
}
...
async function ensureLoaded(): Promise<SqlJsModule> {
  ...
  // `sql.js`'s default export is async-init'd; we use top-level await here.
...
export const repository = {
  ...
  async reset() {
    const m = await ensureLoaded();
    m.db.run("DELETE FROM audit_events; DELETE FROM board_items; DELETE FROM workflow_runs; DELETE FROM workflow_templates; DELETE FROM boards; DELETE FROM integration_credentials; DELETE FROM workspaces;");
    m.persist();
  },
  ...
  async upsertCredential(credential: IntegrationCredential) {
    const m = await ensureLoaded();
    m.db.run(
      ...
      [credential.id, credential.workspaceId, credential.provider, credential.credentialLabel, json(credential.scopes), credential.expiresAt, credential.status],
    );
    m.persist();
  },
  ...
  async upsertTemplate(template: WorkflowTemplate) {
    const m = await ensureLoaded();
    m.db.run(
  ...
  async createBoardItem(input: Omit<BoardItem, "id" | "createdAt">) {
    const m = await ensureLoaded();
  ...
  async updateBoardSync(boardId: string) { run("UPDATE boards SET last_synced_at = ? WHERE id = ?", [now(), boardId]); return this.getBoard(boardId); },
